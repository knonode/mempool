...
((function ( $ ) {
  ...
  $.widget('aerolab.blockrain', {
    ...
    gameover: function() {
      ...
      this.options.onGameOver.call(this.element, 0);
    },
    ...
    _SetupFilled: function() {
      ...
      this._filled = {
        ...
        toClear: {},
        _mainGame: game, // Reference to main game object
        check: function(x, y) {
        ...
        checkForClears: function() {
          ...
          // Flash the filled lines before clearing them
          if (rows.length > 0) {
            this._flashLines(rows);
            return; // Don't clear yet, wait for flash animation
          }

          var clearedLines = game._board.lines - startLines;
          
          // Trigger onLine callback if lines were cleared
          if (clearedLines > 0) {
            game.options.onLine.call(game.element, clearedLines, 0, 0);
          }
        },

        _flashLines: function(rows) {
          var filled = this;
          var mainGame = filled._mainGame; // Reference to main game object
          var flashCount = 0;
          var maxFlashes = 6; // More flashes for more drama
          var flashInterval = 100; // Faster flashing
          
          var flashAnimation = function() {
            flashCount++;
            
            // Toggle visibility of the filled lines with pulsing effect
            for (var i = 0; i < rows.length; i++) {
              var row = rows[i];
              for (var x = 0; x < mainGame._BLOCK_WIDTH; x++) {
                var index = filled.asIndex(x, row);
                if (filled.data[index]) {
                  // Toggle the flash state and add pulse intensity
                  filled.data[index].flashing = !filled.data[index].flashing;
                  filled.data[index].flashIntensity = flashCount; // Track flash intensity
                }
              }
            }
            
            // Force a re-render
            if (mainGame && mainGame._board) {
              mainGame._board.renderChanged = true;
            }
            
            if (flashCount < maxFlashes) {
              // Continue flashing
              setTimeout(flashAnimation, flashInterval);
            } else {
              // Clear the lines after flashing
              for (var i = 0; i < rows.length; i++) {
                filled._popRow(rows[i]);
                mainGame._board.lines++;
                if (mainGame._board.lines % 10 == 0 && mainGame._board.dropDelay > 1) {
                  mainGame._board.dropDelay *= 0.9;
                }
              }
              
              var clearedLines = rows.length;
              if (clearedLines > 0) {
                mainGame.options.onLine.call(mainGame.element, clearedLines, 0, 0);
              }
            }
          };
    ...
    _SetupBoard: function() {
      ...
      this._board = {
        ...
        drawBlock: function(x, y, blockType, blockVariation, blockIndex, blockRotation, falling) {
          ...
          // Check if this block is flashing
          var isFlashing = false;
          var flashIntensity = 0;
          if (blockType && blockVariation !== undefined && blockIndex !== undefined) {
            var dataIndex = game._filled.asIndex(x / game._block_size, y / game._block_size);
            if (game._filled.data[dataIndex] && game._filled.data[dataIndex].flashing) {
              isFlashing = true;
              flashIntensity = game._filled.data[dataIndex].flashIntensity || 0;
            }
          }
          ...
          else if( typeof color === 'string' )
          {
            ...
            if (isFlashing) {
              // Flash with bright white color and glow effect
              game._ctx.fillStyle = '#ffffff';
              game._ctx.globalAlpha = 1.0;
              
              // Add glow effect that intensifies with each flash
              var glowColors = ['#ffff00', '#ff8800', '#ff0088', '#8800ff', '#0088ff', '#00ff88'];
              game._ctx.shadowColor = glowColors[flashIntensity % glowColors.length] || '#ffff00';
              game._ctx.shadowBlur = 15 + (flashIntensity * 2); // Increasing glow
              game._ctx.shadowOffsetX = 0;
              game._ctx.shadowOffsetY = 0;
            } else {
              game._ctx.fillStyle = color;
              game._ctx.globalAlpha = 1.0;
              game._ctx.shadowBlur = 0; // Reset shadow
            }
            game._ctx.fillRect(x, y, game._block_size, game._block_size);
